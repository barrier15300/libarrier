...
namespace libarrier {
	...
	public:
		...
		public:
			...
			line_type at(size_t idx) const requires(is_const) {
				return m_reader->m_lines.at(m_begin + idx);
			}

			auto begin() requires(!is_const) {
				return m_reader->m_lines.begin() + m_begin;
			}
			auto end() requires(!is_const) {
			...
			auto rbegin() requires(!is_const) {
				return std::reverse_iterator(end());
			}
			auto rend() requires(!is_const) {
			...
			size_t line_of(size_t cursor) const {
				...
				auto it = std::lower_bound(begin(), end(), sub.substr(cursor, 1),
